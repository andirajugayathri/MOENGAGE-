...
document.getElementById('saveFilterBtn').addEventListener('click', () => {
    ...
    if (filterName) {
        // Get the image URL of the first filtered status code
        const firstStatus = httpStatuses.find(status => 
            (currentFilter.category === 'all' || Math.floor(status.code / 100) === parseInt(currentFilter.category.charAt(0))) &&
            (currentFilter.search === '' || status.code.toString().includes(currentFilter.search) || status.message.toLowerCase().includes(currentFilter.search))
        );
        const imageUrl = firstStatus ? `https://http.dog/${firstStatus.code}.jpg` : '';

        savedFilters.push({
            name: filterName,
            filter: { ...currentFilter },
            image: imageUrl // Save the image URL along with the filter
        });
    ...
    savedFilters.forEach((saved, index) => {
        ...
        filterItem.innerHTML = `
            <div>
                ...
                ${saved.image ? `<img src="${saved.image}" alt="Status image" style="width: 50px; height: 50px;">` : ''}
            </div>
            <div>
